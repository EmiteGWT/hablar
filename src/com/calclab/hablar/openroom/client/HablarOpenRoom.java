...
public class HablarOpenRoom implements EntryPoint {
    ...
    protected static SimpleAction<RosterPage> createAction(final OpenRoomPage page) {
	final String name = "Open new room";
	final String icon = HablarIcons.get(IconType.buddyWait);
	final SimpleAction<RosterPage> action = new SimpleAction<RosterPage>(name, ACTION_ID, icon) {
	    @Override
	...
	hablar.addPageAddedHandler(new PageAddedHandler() {
		...
		final RosterPage roster = RosterPresenter.asRoster(event.getPage());
		if (roster != null) {
